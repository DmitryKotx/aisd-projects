...
public class SimpleBinaryTree<T> implements BinaryTree<T> {
    ...
    protected class SimpleTreeNode implements BinaryTree.TreeNode<T> {
        ...
        public String path;
        public SimpleTreeNode(T value, SimpleTreeNode left, SimpleTreeNode right, String path) {
            this.value = value;
            this.left = left;
            this.right = right;
            this.path = path;
    ...
    private void setPaths(TreeNode<T> node) {
        if (node == null) {
            return;
        }
        if (node.getLeft() != null) {
            node.getLeft().setPath(node.getPath() + " -> L");
        }
        if (node.getRight() != null) {
            node.getRight().setPath(node.getPath() + " -> R");
        }
        setPaths(node.getLeft());
        setPaths(node.getRight());
    }
