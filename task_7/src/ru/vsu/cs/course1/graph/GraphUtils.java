...
public class GraphUtils {
     ...
     * @return
     */
    public static String toDot(Graph graph, List<Integer> list) {
        StringBuilder sb = new StringBuilder();
        boolean[] vertex = new boolean[graph.vertexCount()];
        ...
        for (int v1 = 0; v1 < graph.vertexCount(); v1++) {
            ...
            for (Integer integer : list) {
                if (integer == v1 && !vertex[v1]) {
                    vertex[integer] = true;
                    sb.append(v1).append("[style = \"filled\", fillcolor = \"green\"]");
                } else {
                    if (!vertex[v1]) {
                        sb.append(v1).append("[style = \"filled\", fillcolor = \"red\"]");
                    }
                }
            }
